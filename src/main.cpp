// iDryer Storage Link — точка входа: меню/NVS → LED-лента → SHT31 → SDK begin()
...
// FastLED требует тип чипсета как шаблонный параметр — задаётся один раз при
// boot (смена требует перезагрузки). chipset: 0=WS2812B 1=WS2811 2=WS2813
// 3=WS2815 4=SK6812 order:   0=RGB     1=RBG    2=GRB    3=GBR    4=BRG 5=BGR

#define ADD_CHIPSET_ORDERED(CHIP, COUNT, ORDER)                                \
  case ORDER:                                                                  \
    FastLED.addLeds<CHIP, STORAGE_LED_PIN, ORDER>(s_leds, COUNT);              \
    break
...
#define ADD_CHIPSET(CHIP, ORDER_IDX, COUNT)                                    \
  do {                                                                         \
    ...
    default:                                                                   \
      FastLED.addLeds<CHIP, STORAGE_LED_PIN, GRB>(s_leds, COUNT);              \
      break;                                                                   \
    }                                                                          \
  } while (0)
...
static void initLedStrip(uint8_t chipset, uint8_t order, uint16_t count) {
  ...
  switch (chipset) {
  case 0:
    ADD_CHIPSET(WS2812B, order, count);
    break;
  case 1:
    ADD_CHIPSET(WS2811, order, count);
    break;
  case 2:
...
static void registerCommands() {
  ...
  s_link.onCommand("invoke", [](JsonObjectConst data) {
    // TODO: унифицировать invoke с iHeater Link (menu id + args) при миграции
    // сушилки.
...
static void cmdStatus() {
  Serial.printf("[status] wifi=%d ip=%s rssi=%d online=%d serial=%s\n",
                ...
                WiFi.RSSI(), s_link.isOnline() ? 1 : 0, s_link.serial());
}
...
void setup() {
  ...
  uint16_t ledCount =
      (menu.led_count > STORAGE_MAX_LEDS) ? STORAGE_MAX_LEDS : menu.led_count;
...
#ifdef IDRYER_DEV_REPL
  // Разбор ввода: завершение по CR/LF или паузе 120 мс (для терминалов без
  // «конца строки»).
  static String buf;
