/**
 * @file types.h
 * @brief Базовые структуры и callback-типы.
 */
...
struct DeviceIdentity {
    ...
    /// Обрезает по размеру буфера и всегда ставит `\0`.
    void setSerialNumber(const char* src) {
        if (src) {
            strncpy(serialNumber, src, sizeof(serialNumber) - 1);
    ...
    void setToken(const char* src) {
        if (src) {
            strncpy(token, src, sizeof(token) - 1);
    ...
    void setDeviceId(const char* src) {
        if (src) {
            strncpy(deviceId, src, sizeof(deviceId) - 1);
...
struct WifiCredentials {
    ...
    void setPassword(const char* src) {
        if (src) {
            strncpy(password, src, sizeof(password) - 1);
...
/// Безопасное копирование в фиксированный буфер.
