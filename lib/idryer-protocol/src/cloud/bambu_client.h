 ...
 *    подписка на `device/<printerSerial>/report` (Фаза 4).
 *
 * Эта реализация — Фаза 2: только Writer-режим.
 *
...
public:
    ...
    /// Для iHeater (`deviceType == Heater / IHeaterLink`) прошивка / Manager
    /// должны выставить `Reader` до `configure()` — это влияет на то,
    /// будет ли клиент подписываться на `device/<serial>/report`.
    ...
    /// После `shutdown()` новый `configure()` снова заработает.
    void shutdown();
    ...
    /// Прикладная прошивка LINK iHeater использует его для передачи
    /// данных MCU через UART.
    void setPrinterStatusCallback(PrinterStatusCallback cb) { printerStatusCallback_ = std::move(cb); }

    /// Последний снимок статуса принтера (доступен в обоих режимах,
    /// но в Writer-режиме заполняется только подпиской если Manager
    /// решил её оставить — по умолчанию в Writer подписки на report нет).
    const BambuPrinterStatus& printerStatus() const { return printerStatus_; }

    // Writer API --------------------------------------------------------------

    /// Послать `ams_filament_setting` в принтер.
    /// `amsId`/`trayId` со спец-значениями `0xFF`/`0xFE` замещаются
    /// значениями из текущего `BambuConfig`.
    /// Возвращает результат + заполняет `lastError` при неудаче.
    BambuApplyResult applyFilament(const BambuApplyPayload& payload);

private:
    static constexpr uint32_t kReconnectMinMs  = 1000;     // 1 секунда
    static constexpr uint32_t kReconnectMaxMs  = 60000;    // 60 секунд
    ...
    // Обработчик входящих MQTT-сообщений (Reader-режим).
    void handleReportMessage(const char* topic, const uint8_t* payload, unsigned int length);

    // Запрос полного снимка состояния принтера сразу после connect (Reader).
    void requestPushAll();
    ...
    // Топики: `device/<serial>/request` (publish) и `device/<serial>/report` (subscribe).
    char                    requestTopic_[48] = {0};
    char                    reportTopic_[48]  = {0};

    // Последний известный снимок из report.
