...
void HaIntegrationAdapter::configure(const HaConfig& cfg)
{
    ...
    // Любое изменение — дисконнект и реконфигурация с нуля.
    if (client_.isConnected()) {
        // HaMqttClient не имеет public disconnect(); он обслуживается
        // через loop() и PubSubClient. Просто не зовём loop до следующего
        // attemptConnect — при следующем connect создастся новая сессия.
    }
...
void HaIntegrationAdapter::attemptConnect()
{
    ...
    // Резолвим брокера: если host — IPv4 → setServer напрямую, иначе
    // discover() (mDNS / DNS).
    IPAddress ip;
    if (isIpAddress(cfg_.host) && ip.fromString(cfg_.host)) {
        client_.setServer(ip, cfg_.port);
    } else {
        // discover принимает hostname, внутри делает mDNS, либо фолбэк
        // на обычное WiFi.hostByName().
        if (!client_.discover(cfg_.host)) {
            setError("discover failed (HA not reachable)");
            // Растим backoff — возможно WiFi в роуминге или HA ещё не поднят.
    ...
    // Аутентификация: username/password опциональны. Если username пуст —
    // подключаемся анонимно (HaMqttClient::connect обрабатывает nullptr).
...
{
    if (!s || !*s) return false;
    // Простая проверка: только цифры и точки.
