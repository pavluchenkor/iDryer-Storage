    ...
    namespace ha
    {
        ...
        HaMqttClient::HaMqttClient()
        {
            ...
            mqttClient_.setCallback([this](char *topic, uint8_t *payload, unsigned int length)
                                    {
        if (messageCallback_) {
            // null-terminate payload во временном буфере
            char buf[256];
        ...
        bool HaMqttClient::discover(const char* host)
        {
            ...
            if (host && host[0] != '\0') {
                ...
                // Фолбэк для .local: WiFi.hostByName() на ESP32 не резолвит mDNS-имена.
                //
                // Под капотом hostByName() = lwip_gethostbyname() шлёт unicast DNS на
                // DHCP-выданный сервер (роутер), который про .local ничего не знает.
                // В ESP-IDF есть опция CONFIG_LWIP_DNS_SUPPORT_MDNS_QUERIES, которая
                // должна делать mDNS-multicast из gethostbyname(), но: (1) в arduino-esp32
                // core она не гарантированно включена, (2) даже когда включена —
                // шлёт "QM" (multicast) запросы, на которые mDNS-респондеры не отвечают,
                // в отличие от "QU" (unicast) запросов через mdns_query_a() напрямую.
                // Это не специфика Arduino-обёртки: на чистом ESP-IDF тот же баг.
                //
                // Единственный надёжный путь — вызвать MDNS.queryService(), которая
                // обёртка над espressif/mdns → mdns_query_ptr() + mdns_query_a().
                //
                // Refs:
                ...
                //   https://github.com/espressif/arduino-esp32/issues/1259
                //   https://github.com/espressif/arduino-esp32/issues/10613
                if (haIP == INADDR_NONE) {
                ...
                // Автообнаружение через mDNS service discovery (_home-assistant._tcp).
                // Не вызываем MDNS.begin() — он уже инициализирован WsServer с serial-именем.
                // Повторный begin() перезапишет hostname и сломает mDNS-рекламу устройства.
                // Про WiFi.hostByName("homeassistant.local") как фолбэк см. комментарий
                // в ветке выше — на ESP32 этот путь не работает, убран как dead code.
                delay(200);
                int n = MDNS.queryService("home-assistant", "tcp");
                                   ...
                                   const char *password)
        {
            ...
            if (username && username[0] != '\0')
            {
                ...
                HAL_LOG_INFO("HA_MQTT", "Connecting with auth: %s@%s:%d",
                             clientId_,
                             discoveryResult_.ip.toString().c_str(),
